...
cv::Mat to_hatched(const cv::Mat frame)
{
  ...
  auto reduced = cv::Mat();
  cv::resize(gray, reduced, cv::Size(gray.cols / 4, gray.rows / 4));

  auto c_black = cv::Scalar(0, 0, 0);
  auto line_offset = 4;

  reduced.forEach<uchar>([&](uchar &pixel, const int *position) -> void {
    auto [x, y] = std::make_tuple(position[1], position[0]);
    auto [x_origin, y_origin] = std::make_tuple(x * 4, y * 4);
    auto val = pixel;
    ...
    if (val < 32)
    {
      cv::line(
          //
          frame_out, cv::Point(x_origin, y_origin), cv::Point(x_origin + line_offset, y_origin + line_offset), c_black,
          1, 8);
      cv::line(
          //
          frame_out, cv::Point(x_origin, y_origin + line_offset),
          cv::Point(x_origin + line_offset, y_origin + line_offset), c_black,
          //
          1, 8);
      cv::line(
          //
          frame_out, cv::Point(x_origin + line_offset, y_origin),
          cv::Point(x_origin + line_offset, y_origin + line_offset), c_black,
          //
          1, 8);
    }
...
cv::Mat process(const cv::Mat &frame, const LookupTable &lookupTable)
{
  ...
  // ハッチング
  frame_out = to_hatched(frame_out);
